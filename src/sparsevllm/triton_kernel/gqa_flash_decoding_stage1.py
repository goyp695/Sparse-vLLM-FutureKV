...
@triton.jit
def _fwd_kernel_flash_decode_stage1_head_slots(
    Q,
    K,
    V,
    sm_scale,
    ...
    for start_n in tl.range(0, block_n_size, 1):
        ...
        k_loc = tl.load(
            Head_slots
            + cur_batch * stride_head_slots_b
            + cur_kv_head * stride_head_slots_h
            + offs_n_new * stride_head_slots_s,
            mask=offs_n_new < cur_batch_end_index,
            other=0,
        )
        off_k = k_loc[None, :] * stride_kbs + cur_kv_head * stride_kh + offs_d[:, None]
        k = tl.load(K + off_k, mask=offs_n_new[None, :] < cur_batch_end_index, other=0.0)
        att_value = tl.dot(q, k) * sm_scale
        att_value = tl.where(offs_n_new[None, :] < cur_batch_end_index, att_value, float("-inf"))
        v = tl.load(
            V + k_loc[:, None] * stride_vbs + cur_kv_head * stride_vh + offs_d[None, :],
            mask=offs_n_new[:, None] < cur_batch_end_index,
            other=0.0,
        )
...
@torch.no_grad()
def flash_decode_stage1_head_slots(
    q, k, v, Head_slots, B_Seqlen, max_len_in_batch, mid_out, mid_out_logsumexp, block_seq
):
    BLOCK_SEQ = block_seq
    BLOCK_N = 16
    assert BLOCK_SEQ % BLOCK_N == 0
    ...
    assert Lk in {16, 32, 64, 128}
    sm_scale = 1.0 / (Lk ** 0.5)
    batch, kv_head_num = B_Seqlen.shape[0], k.shape[1]
    grid = (batch, kv_head_num, triton.cdiv(max_len_in_batch, BLOCK_SEQ))
    gqa_group_size = q.shape[1] // k.shape[1]

    _fwd_kernel_flash_decode_stage1_head_slots[grid](
        q,
        k,
        v,
        sm_scale,
